...
const bot = () => {
  ...
  form.addEventListener('submit', e => {
    ...
    const addinform = formData.get('addinform');
    const messageText = `
            Нове замовлення!!!
            Ім*я: ${firstName};
            Прізвище: ${secondName};
            Телефон: ${number};
            Місто: ${city};
            Відділення нової пошти: ${department};
            Додаткова інформація: ${addinform};
        `;
    const payload = {
      chat_id: chatId,
      text: messageText
    ...
    }).then(response => response.json()).then(data => console.log(data)).catch(error => console.error('Error sending message:', error)).finally(() => {
      form.reset();
    });
  });
};
...
/* harmony export */ });
const burger = () => {
  const burgerLineFirst = document.querySelector('.header__burger-line-firs'),
    burgerLineSecond = document.querySelector('.header__burger-line-second'),
    burgerLineThird = document.querySelector('.header__burger-line-third'),
    burger = document.querySelector('.header__burger'),
    menu = document.querySelector('.header__menu');
  burger.addEventListener('click', () => {
    if (!menu.classList.contains('header__active-menu')) {
      menu.classList.add('header__active-menu');
      burgerLineFirst.classList.add('header__burger-line-first-active');
      burgerLineSecond.classList.add('header__burger-line-second-active');
      burgerLineThird.classList.add('header__burger-line-third-active');
    } else {
      menu.classList.remove('header__active-menu');
      burgerLineFirst.classList.remove('header__burger-line-first-active');
      burgerLineSecond.classList.remove('header__burger-line-second-active');
      burgerLineThird.classList.remove('header__burger-line-third-active');
    }
  });
};
...
const slider = () => {
  const buttons = document.querySelectorAll('.splide__pagination__page'),
    arrows = document.querySelectorAll('.splide__arrow'),
    total = document.getElementById('total'),
    current = document.getElementById('current'),
    slides = document.querySelectorAll('.splide__slide img');
  total.textContent = `0${buttons.length}`;
  current.textContent = `0${1}`;
  const addClassToActivePagination = () => {
    buttons.forEach(button => {
      if (button.classList.contains('is-active')) {
        button.closest('li').style.opacity = '1';
      }
    });
    buttons.forEach((button, i) => {
      button.addEventListener('click', () => {
        buttons.forEach(button => button.closest('li').style.opacity = '0.5');
        if (button.classList.contains('is-active')) {
          button.closest('li').style.opacity = '1';
